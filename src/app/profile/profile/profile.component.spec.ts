...
@Component({
  ...
  template: `
    <div id="follow" (click)="onFolowing()"></div>
  `
})
...
@Component({
  ...
  template: `
    <div id="tabs" (click)="onSelectedType($event)"></div>
  `
})
...
@Component({
  ...
  template: `
    <div id="favorite" (click)="onFavorited()"></div>
  `
})
...
@Component({
  ...
  template: `
    <div id="page" (click)="onPageChange($event)"></div>
  `
})
...
describe('ProfileComponent', () => {
  ...
  it('should select articlesCount', done => {
    component.articlesCount$.subscribe(
      res => {
        expect(res).toBe(0);
        done();
      },
  ...
  it('should select loadingArticles', done => {
    component.loadingArticles$.subscribe(
      res => {
        expect(res).toBeFalsy();
        done();
      },
  ...
  it('should dispatch "ResetConfig, SetAuthor, LoadArticlesRequest" if there is no return articles config', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.ResetConfig()
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetAuthor({ author: 'username' })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.LoadArticlesRequest()
    );
  });
  ...
  it('should dispatch set articles config if there is return articles config', () => {
    store.dispatch(new ProfileActions.ProfileLoadSuccess({ profile }));
    store.dispatch(
      new ArticlesActions.SetReturnArticlesConfig({ config: returnConfig })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.ResetConfig()
    );
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetAuthor({ author })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetFavorited({ favorited })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetLimit({ limit })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetOffset({ offset })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetPageIndex({ pageIndex: offset / limit })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.LoadArticlesRequest()
    );
  ...
  it('should handle ngOnDestroy', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ProfileActions.ProfileClear()
    );
  ...
  it('should dispatch "ProfileToggleFollowingRequest"', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ProfileActions.ProfileToggleFollowingRequest({ profile })
    );
  });
  ...
  it('should dispatch "ToggleArticleFavoriteRequest"', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.ToggleArticleFavoriteRequest({ article })
    );
  });
  ...
  it('should load article for sertain tab and set favorited', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetFavorited({ favorited: 'username' })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.LoadArticlesRequest()
    );
  ...
  it('should change page', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetLimit({ limit: page.pageSize })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetPageIndex({ pageIndex: page.pageIndex })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.SetOffset({ offset: page.pageSize * page.pageIndex })
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      new ArticlesActions.LoadArticlesRequest()
    );
