...
export class ArticleEffects {
  ...
  deleteArticle$ = this.actions$.pipe(
    ofType<ArticleDeleteConfirmation>(
      ArticleActionTypes.ArticleDeleteConfirmation
    ),
  ...
  toggleFollowUserArticle$ = this.actions$.pipe(
    ofType<ArticleToggleFollowingRequest>(
      ArticleActionTypes.ArticleToggleFollowingRequest
    ),
  ...
  articleFollowing$ = this.actions$.pipe(
    ...
    mergeMap(([action, favoritingProfile]) => {
      ...
      return this.profileService.follow(username).pipe(
        map(result => {
          return new ArticleToggleFollowingSuccess({
        ...
        catchError(error => {
          console.error(error);
          return of(
            new ArticleToggleFollowingFail({
              profile,
              showNotification: !favoritingProfile
            })
          );
  ...
  aricleUnFollowing$ = this.actions$.pipe(
    ofType<ArticleUnFollowingRequest>(
      ArticleActionTypes.ArticleUnFollowingRequest
    ),
    mergeMap(action => {
      const { profile } = action.payload;
      const { username } = profile;
  ...
  articleToggleFollowingSuccess$ = this.actions$.pipe(
    ofType<ArticleToggleFollowingSuccess>(
      ArticleActionTypes.ArticleToggleFollowingSuccess
    ),
    filter(action => action.payload && action.payload.showNotification),
    map(action => action.payload.profile),
    switchMap((profile: Profile) =>
      this.translaService.get('conduit.profile.follow.succes', {
  ...
  articleToggleFollowingFail$ = this.actions$.pipe(
    ofType<ArticleToggleFollowingFail>(
      ArticleActionTypes.ArticleToggleFollowingFail
    ),
    filter(action => action.payload && action.payload.showNotification),
    map(action => action.payload.profile),
    switchMap((profile: Profile) =>
      this.translaService.get('conduit.profile.follow.fail', {
        value: profile.username
      })
    ),
    tap((notification: string) =>
  ...
  loadComments$ = this.actions$.pipe(
    ...
    switchMap(slug =>
      this.commentsService.getAll(slug).pipe(
  ...
  addComment$ = this.actions$.pipe(
    ofType<ArticleCommentAddRequest>(
      ArticleActionTypes.ArticleCommentAddRequest
    ),
    withLatestFrom(this.store.pipe(select(selectArticle))),
    mergeMap(([action, article]) => {
      const { slug } = article;
      return this.commentsService.add(slug, action.payload.comment).pipe(
  ...
  deleteComment$ = this.actions$.pipe(
    ofType<ArticleCommentDeleteRequest>(
      ArticleActionTypes.ArticleCommentDeleteRequest
    ),
    withLatestFrom(this.store.pipe(select(selectArticle))),
    mergeMap(([action, article]) => {
      const { slug } = article;
      const { id } = action.payload.comment;
      return this.commentsService.destroy(id, slug).pipe(
