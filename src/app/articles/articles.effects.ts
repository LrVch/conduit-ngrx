...
export class ArticlesEffects {
  ...
  loadArticles$ = this.actions$.pipe(
    ...
    switchMap(([action, filters]) => {
      return this.articlesService.query(filters).pipe(
        switchMap(response => {
          const {
            entities,
            result
          }: NormalizedArticlesResponse = ArticlesCashService.normalizeArticlesResponce(
            response
          );
  ...
  loadTags$ = this.actions$.pipe(
    ...
    switchMap(
      ([action, tags]): Observable<LoadTagsLoaded | LoadTagsFail> => {
        if (tags.length) {
  ...
  taggleFavoriteArticle$ = this.actions$.pipe(
    ofType<ToggleArticleFavoriteRequest>(
      ArticlesActionTypes.ToggleArticleFavoriteRequest
    ),
    withLatestFrom(this.store.pipe(select(selectAuthLoggedIn))),
    mergeMap(
      ([action, isLoggedIn]): Observable<
        UnFavoriteArticleRequest | FavoriteArticleRequest | SetFavoritingArticle
      > => {
        if (!isLoggedIn) {
          return of(
            new SetFavoritingArticle({ article: action.payload.article })
          );
        }
  ...
  unFavoriteArticle$ = this.actions$.pipe(
    ofType<UnFavoriteArticleRequest>(
      ArticlesActionTypes.UnFavoriteArticleRequest
    ),
    mergeMap(action => {
      const { article } = action.payload;
      const { slug } = article;

      return this.articlesService.unfavorite(slug).pipe(
        map(res => {
  ...
  favoriteArticle$ = this.actions$.pipe(
    ...
    mergeMap(([action, favoritingArticle]) => {
      ...
      return this.articlesService.favorite(slug).pipe(
        map(res => {
          const art = res.article;
          return new ToggleArticleFavoriteSuccess({
            article: art,
            showNotification: !favoritingArticle
          });
        }),
        retry(3),
        catchError(error => {
  ...
  toggleArticleFavoriteSuccess$ = this.actions$.pipe(
    ofType<ToggleArticleFavoriteSuccess>(
      ArticlesActionTypes.ToggleArticleFavoriteSuccess
    ),
    filter(action => action.payload.showNotification),
    map(action => action.payload.article),
    switchMap((article: Article) =>
      this.translateService.get('conduit.article.favorite.success', {
        value: article.title
      })
    ),
  ...
  toggleArticleFavoriteFail$ = this.actions$.pipe(
    ofType<ToggleArticleFavoriteFail>(
      ArticlesActionTypes.ToggleArticleFavoriteFail
    ),
    filter(action => action.payload.showNotification),
    map(action => action.payload.article),
    switchMap((article: Article) =>
      this.translateService.get('conduit.article.favorite.fail', {
        value: article.title
      })
    ),
    tap((notification: string) =>
  ...
  deleteArticle$ = this.actions$.pipe(
    ofType<ArticlesDeleteArticleConfirmation>(
      ArticlesActionTypes.ArticlesDeleteArticleConfirmation
    ),
    map(action => action.payload.article),
    mergeMap(article => {
      const { slug } = article;
      return this.articlesService.destroy(slug).pipe(
  ...
  articlesDeleteArticleFail$ = this.actions$.pipe(
    ofType<ArticlesDeleteArticleFail>(
      ArticlesActionTypes.ArticlesDeleteArticleFail
    ),
    map(action => action.payload.article),
    switchMap((article: Article) =>
      this.translateService.get('conduit.article.delete.fail', {
        value: article.title
      })
    ),
    tap((notification: string) =>
  ...
  articlesDeleteArticleSuccess$ = this.actions$.pipe(
    ofType<ArticlesDeleteArticleSuccess>(
      ArticlesActionTypes.ArticlesDeleteArticleSuccess
    ),
    map(action => action.payload.article),
    switchMap((article: Article) =>
      this.translateService.get('conduit.article.delete.success', {
        value: article.title
      })
    ),
    tap((notification: string) =>
