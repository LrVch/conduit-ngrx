...
export class AppSettingsEffects {
  ...
  @Effect()
  setInitLanguage$ = this.store
    .pipe(select(selectAppSettingsDefaultLanguage))
    .pipe(
      withLatestFrom(
        this.store.pipe(select(selectAppSettingsStateLanguages)),
        this.store.pipe(select(selectAppSettingsStateLanguage))
      ),
  ...
  setTitle$ = merge(
    this.actions$.pipe(
      ofType(AppSettingsActionTypes.AppSettingsChangeLanguage)
    ),
  ...
  updateEffectiveTheme$ = this.store.pipe(
    ...
    distinctUntilChanged(),
    map(
      effectiveTheme => new AppSettingsUpdateEffectiveTheme({ effectiveTheme })
    )
  );
  ...
  updateRouteAnimationType$ = merge(
    INIT,
    this.actions$.pipe(
      ofType(AppSettingsActionTypes.AppSettingsChangeRouteAnimationType)
    )
  ).pipe(
    withLatestFrom(
      this.store.pipe(select(selectAppSettingsRouteAnimationChangeType))
    ),
    tap(([action, type]) =>
  ...
  updateRouteAnimationEnabled$ = merge(
    INIT,
    this.actions$.pipe(
      ofType(AppSettingsActionTypes.AppSettingsChangeRouteAnimationEnabled)
    )
  ).pipe(
    withLatestFrom(
      this.store.pipe(select(selectAppSettingsRouteAnimationChangeEnabled))
    ),
    tap(([action, enabled]) =>
